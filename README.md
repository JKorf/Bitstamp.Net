...
V2 uses a strict interface per capability. The `SharedApi` property on each API surface therefore exposes only the operations or subscriptions that surface actually supports. `IBitstampSharedApiClient` groups the exchange's Shared API surfaces for dependency injection and runtime capability lookup.
...
using var restClient = new BitstampRestClient();
...
The request and response models belong to `CryptoExchange.Net.SharedApis`, so the same operation can accept another exchange's `IGetTickerRest` implementation. When using dependency injection, inject `IBitstampSharedApiClient` to access all of the exchange's Shared API surfaces or inject a capability such as `IGetTickerRest` directly. Use `GetCapability` on the aggregate when the operation, transport, or trading mode is selected at runtime.
...
* Version 2.5.0 - 24 Sep 2026
    ...
    * Shared APIs
      ...
      * Added `(I)BitstampSharedApiClient` containing all V2 SharedApi references
      * Added `SharedApi` to `BitstampOptions` to provide SharedApi related options
    ...
    * Added ExchangApi.SubscribeToSelfTradeUpdatesAsync subscription
...
* Version 2.2.0 - 21 Jul 2026
    ...
    * Fixed CancelAllOrdersAsync paths
